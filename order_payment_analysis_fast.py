...
"""
Быстрый анализ связи чатов с заказами и оплатой.

Оптимизированная версия: кеширует заказы по client_id, чтобы не делать повторные запросы.
"""
...
def get_orders_by_customer(client: RetailCrmClient, customer_id: str, limit: int = 20, max_pages: int = 5) -> List[Dict[str, Any]]:
    ...
    while page <= max_pages:
        try:
            ...
            # Если ошибка API - прерываем
            print(f"      ⚠️ RetailCRM ошибка на странице {page}: {str(e)[:80]}")
            break
        except Exception as e:
            print(f"      ⚠️ Ошибка на странице {page}: {str(e)[:80]}")
            break
...
def find_related_order(orders: List[Dict[str, Any]], chat_created_at: Optional[datetime], days_window: int = 30) -> Optional[Dict[str, Any]]:
    ...
    # Нормализуем дату чата (убираем timezone если есть)
    if chat_created_at.tzinfo:
        chat_created_at = chat_created_at.replace(tzinfo=None)
    ...
    for order in orders:
        ...
        if order_created:
            ...
            if chat_created_at <= order_created <= window_end:
...
def main() -> None:
    ...
    # Ограничение для теста (по умолчанию 10)
    TEST_LIMIT = os.environ.get("TEST_LIMIT", "10")
    if TEST_LIMIT:
    ...
    print(f"Загружено чатов для анализа: {len(chats)}")
    
    # Уменьшаем таймаут для быстрой работы
    client = RetailCrmClient.from_env()
    client.timeout_s = 30  # 30 секунд вместо 60
    
    # КЕШИРУЕМ заказы по client_id (чтобы не делать повторные запросы)
    print("\nСобираю уникальные client_id...")
    ...
    print("Загружаю заказы из RetailCRM (это может занять время)...")
    
    orders_cache: Dict[str, List[Dict[str, Any]]] = {}
    processed_clients = 0
    
    print(f"Начинаю загрузку заказов для {len(unique_client_ids)} уникальных клиентов...")
    ...
    for idx, client_id in enumerate(unique_client_ids, 1):
        try:
            ...
            orders = get_orders_by_customer(client, client_id, limit=20, max_pages=3)
            elapsed = time.time() - start_time
            orders_cache[client_id] = orders
            print(f" ✅ {len(orders)} заказов ({elapsed:.1f}с)")
            processed_clients += 1
        except RetailCrmError as e:
            print(f" ❌ RetailCRM: {str(e)[:60]}")
            orders_cache[client_id] = []
        except Exception as e:
    ...
    # Анализируем чаты
    print("\nАнализирую связь чатов с заказами...")
    results = []
    ...
    for i, chat in enumerate(chats):
        ...
        if client_id and client_id.strip():
            ...
            if orders:
                ...
                if related_order:
                    ...
                    total_sum = related_order.get("totalSumm") or 0
                    prepay_sum = related_order.get("prepaySum") or 0
                    purchase_sum = related_order.get("purchaseSumm") or 0
        ...
        if (i + 1) % 100 == 0:
            print(f"  Обработано чатов: {i + 1}/{len(chats)}")
    ...
    upsert_worksheet(
        ...
        rows=dicts_to_table(
            ...
            header=[
                ...
                "chats_paid", "chats_successful", "success_rate",
            ],
        ),
    )
    
    print("\n✅ Анализ завершён!")
    print(f"   - chat_order_payment ({len(results)} строк)")
    print(f"   - manager_order_payment_stats ({len(manager_rows)} строк)")
